...
AddComponentAction("SCENE", "ghostbabysitter", function(inst, doer, actions, right)
...
AddComponentAction("SCENE", "ghostgestalter", function(inst, doer, actions, right)
...
AddComponentAction("USEITEM", "gravedigger", function(inst, doer, target, actions)
    if target:HasTag("gravediggable") and doer:HasTag("gravedigger_user") then
        table.insert(actions, ACTIONS.GRAVEDIG)
    end
end)

AddComponentAction("EQUIPPED", "gravedigger", function(inst, doer, target, actions, right)
    if right and target:HasTag("gravediggable") and doer:HasTag("gravedigger_user") then
        table.insert(actions, ACTIONS.GRAVEDIG)
    end
end)

AddComponentAction("USEITEM", "ghostlyelixir", function(inst, doer, target, actions)
    if target:HasTag("elixir_drinker") then
        table.insert(actions, ACTIONS.APPLYELIXIR)
    end
end)

AddComponentAction("INVENTORY", "ghostlyelixir", function(inst, doer, actions, right)
