...
local function StartPreviewChannelCast(inst, buffaction)
    if buffaction.action == ACTIONS.START_CHANNELCAST then
        ...
        inst.components.locomotor:SetPredictExternalSpeedMultiplier(inst, "preview_channelcast",
            TUNING.CHANNELCAST_SPEED_MOD)
    elseif buffaction.action == ACTIONS.STOP_CHANNELCAST then
        ...
        inst.components.locomotor:SetPredictExternalSpeedMultiplier(inst, "preview_channelcast",
            1 / TUNING.CHANNELCAST_SPEED_MOD)
    else
        StopPreviewChannelCast(inst)
        return
    end
...
local actionhandlers =
{
    ...
    ActionHandler(ACTIONS.DEPLOY,
        function(inst, action)
            return action.invobject and action.invobject:HasTag("projectile") and "throw_deploy" or
                "doshortaction"
        end),
    ...
    ActionHandler(ACTIONS.CONSTRUCT,
        function(inst, action)
            return (action.target == nil or not action.target:HasTag("constructionsite")) and "startconstruct" or
                "construct"
        end),
...
local states =
{
    ...
    State {
        ...
        onupdate = function(inst)
            ...
            elseif inst.bufferedaction == nil then
                inst.AnimState:PlayAnimation(inst.sg.statemem.state == "mighty" and "dumbbell_mighty_pst" or
                    "dumbbell_normal_pst")
            ...
            if inst.bufferedaction ~= nil then
                ...
                if flower ~= nil and flower:IsValid() then
                    if flower.skin_id ~= 0 then
                        inst.AnimState:OverrideItemSkinSymbol("flower", flower.AnimState:GetBuild(), "flower",
    ...
    State {
        ...
        onupdate = function(inst)
            ...
            elseif inst.bufferedaction == nil then
                inst.AnimState:PlayAnimation(inst.sg.statemem.channelcastitem and "channelcast_idle_pre" or
    ...
    State {
        ...
        onenter = function(inst)
            local anim =
            "lift" --(inst.player_classified ~= nil and inst.player_classified.currentmightiness:value() >= 100) and "lift_full" or "lift"
        ...
        onexit = function(inst)
            ...
            if not inst.sg.statemem.not_interrupted then
                ...
                local radius = inst:GetPhysicsRadius(0) +
                    (chair and chair:IsValid() and chair:GetPhysicsRadius(0) or 0.25)
                if radius > 0 then
                    local x, y, z = inst.Transform:GetWorldPosition()
...
CommonStates.AddHopStates(states, true, hop_anims, hop_timelines, "turnoftides/common/together/boat/jump_on", nil,
    { start_embarking_pre_frame = 4 * FRAMES })
