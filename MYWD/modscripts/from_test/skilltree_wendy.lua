            ...
            onactivate = function(inst, fromload)
                ...
                local addresists = function(pref)
                    ...
                    if damagetyperesist then
                        damagetyperesist:AddResist("shadow_aligned", pref, TUNING.SKILLS.WENDY.ALLEGIANCE_SHADOW_RESIST,
                            "allegiance_shadow")
                    end
                    local damagetypebonus = pref.components.damagetypebonus
                    if damagetypebonus then
                        damagetypebonus:AddBonus("lunar_aligned", pref, TUNING.SKILLS.WENDY.ALLEGIANCE_VS_LUNAR_BONUS,
                            "allegiance_shadow")
                    end
                end
        ...
        wendy_lunar_1 = {
            ...
            onactivate = function(inst, fromload)
                ...
                local addresists = function(pref)
                    local damagetyperesist = pref.components.damagetyperesist
                    if damagetyperesist then
                        damagetyperesist:AddResist("lunar_aligned", pref, TUNING.SKILLS.WENDY.ALLEGIANCE_LUNAR_RESIST,
                            "allegiance_lunar")
                    end
                    local damagetypebonus = pref.components.damagetypebonus
                    if damagetypebonus then
                        damagetypebonus:AddBonus("shadow_aligned", pref, TUNING.SKILLS.WENDY.ALLEGIANCE_VS_SHADOW_BONUS,
                            "allegiance_lunar")
                    end
                end
...
-- MYWD:注册原版技能树，更新后可以删了
local function buildSkillTree(character_name, bf_fn)
    local skill_defs = require("prefabs/skilltree_defs")
    local sdata = bf_fn(skill_defs.FN)

    -- 注册技能树背景图
    -- "images/skilltree_bg.xml"
    RegisterSkilltreeBGForCharacter(resolvefilepath("images/skilltree4.xml"), character_name)

    for _, skill in ipairs(sdata.SKILLS) do
        RegisterSkilltreeIconsAtlas(resolvefilepath("images/skilltree_icons.xml"), skill.icon .. ".tex")
    end




    skill_defs.CreateSkillTreeFor(character_name, sdata.SKILLS)
    skill_defs.SKILLTREE_ORDERS[character_name] = sdata.ORDERS
    -- skill_defs.SKILLTREE_METAINFO["wendy"].BACKGROUND_SETTINGS = data.BACKGROUND_SETTINGS
    -- 构建技能树
end
buildSkillTree("wendy", BuildSkillsData)
