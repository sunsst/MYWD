...
local function DoTalkSound(inst)
    ...
    elseif not inst:HasTag("mime") then
        inst.SoundEmitter:PlaySound(
        (inst.talker_path_override or "dontstarve/characters/") .. (inst.soundsname or inst.prefab) .. "/talk_LP", "talk")
        return true
    end
end
...
local function DoWortoxPortalTint(inst, val)
    ...
    if val > 0 then
        ...
        inst.components.colouradder:PushColour("portaltint", 154 / 255 * val + offset, 23 / 255 * val + offset,
            19 / 255 * val + offset, 0)
        val = 1 - val
...
local function HandleInstrumentAssets(inst, build, symbol)
    ...
    if skin_build ~= nil then
        inst.AnimState:OverrideItemSkinSymbol(symbol, skin_build, override_symbol or symbol, inv_obj.GUID,
            override_build or build)
    else
...
local actionhandlers =
{
    ...
    ActionHandler(ACTIONS.NET,
        function(inst, action)
            ...
            return not inst.sg:HasStateTag("prenet") and (inst.sg:HasStateTag("netting") and "bugnet" or "bugnet_start") or
            nil
        end),
    ...
    ActionHandler(ACTIONS.DEPLOY,
        function(inst, action) return action.invobject and action.invobject.components.complexprojectile and
            "throw_deploy" or "doshortaction" end),
    ...
    ActionHandler(ACTIONS.CASTSUMMON,
        function(inst, action)
            return action.invobject ~= nil and action.invobject:HasTag("abigail_flower") and "summon_abigail" or
            "castspell"
        end),
    ActionHandler(ACTIONS.CASTUNSUMMON,
        function(inst, action)
            return action.invobject ~= nil and action.invobject:HasTag("abigail_flower") and "unsummon_abigail" or
            "castspell"
        end),
    ...
    ActionHandler(ACTIONS.CONSTRUCT,
        function(inst, action)
            return (action.target == nil or action.target.components.constructionsite == nil) and "startconstruct" or
            "construct"
        end),
            ...
            elseif inst.sg:HasStateTag("parrying") and data.redirected then
                if not inst.sg:HasStateTag("parryhit") then
                    ...
                    inst.sg:GoToState("parry_hit", {
                        timeleft = inst.sg.statemem.task ~= nil and GetTaskRemaining(inst.sg.statemem.task) or
                        inst.sg.statemem.parrytime,
                        pushing = data.attacker ~= nil and data.attacker.sg ~= nil and
                        data.attacker.sg:HasStateTag("pushing"),
    ...
    EventHandler("knockback", function(inst, data)
        if not inst.components.health:IsDead() then
            ...
            elseif inst.sg:HasStateTag("parrying") then
                ...
                inst.sg:GoToState(
                (data.forcelanded or inst.components.inventory:EquipHasTag("heavyarmor") or inst:HasTag("heavybody")) and
                "knockbacklanded" or "knockback", data)
            end
        end
    end),
                ...
                if data ~= nil and data.reason ~= nil then
                    if data.reason == "linesnapped" or data.reason == "toofaraway" then
                        ...
                        inst.sg:GoToState("oceanfishing_stop",
                            {
                                escaped_str = data.reason == "linetooloose" and "ANNOUNCE_OCEANFISHING_LINETOOLOOSE"
        ...
        if not (inst.sg:HasStateTag("busy") or
            ...
            if data and data.won then
                ...
                inst.sg:GoToState("emote",
                    { anim = "emoteXL_angry", mounted = true, mountsound = "angry", mountsounddelay = 7 * FRAMES })
            end
        end
    end),
...
local states =
{
    ...
    State {
        ...
        timeline =
        {
            TimeEvent(0 * FRAMES, function(inst) inst.SoundEmitter:PlaySound(
                "wanda2/characters/wanda/younger_transition") end),
        },
    ...
    State {
        ...
        onenter = function(inst)
            ...
            local x, y, z = inst.Transform:GetWorldPosition()
            local flowers = TheSim:FindEntities(x, y, z, DEPLOYSPACING_RADIUS[DEPLOYSPACING.LESS], FLOWERS_MUST_TAGS,
                FLOWERS_CANT_TAGS)
        ...
        onenter = function(inst, pushanim)
            ...
            elseif inst:IsChannelCasting() then
                ...
                if inst.sg.lasttags and inst.sg.lasttags["keepchannelcasting"] then
                    ...
                    table.insert(anims,
                        inst.sg.statemem.channelcastitem and "channelcast_idle_pre" or "channelcast_oh_idle_pre")
                end
        ...
        events =
        {
            ...
            EventHandler("stopchannelcast", function(inst)
                if inst.sg.statemem.channelcast and not inst:IsChannelCasting() then
                    inst.AnimState:PlayAnimation(inst.sg.statemem.channelcastitem and "channelcast_idle_pst" or
                    "channelcast_oh_idle_pst")
                    inst.sg:GoToState("idle", true)
                end
            end),
        },
    ...
    State {
        ...
        onenter = function(inst)
            ...
            elseif inst.customidleanim == nil and inst.customidlestate == nil then
                ...
                local anim = inst.customidleanim ~= nil and
                (type(inst.customidleanim) == "string" and inst.customidleanim or inst:customidleanim()) or nil
                local state = anim == nil and
                (inst.customidlestate ~= nil and (type(inst.customidlestate) == "string" and inst.customidlestate or inst:customidlestate())) or
                nil
            ...
            EventHandler("animover", function(inst)
                if inst.AnimState:AnimDone() then
                    if inst.sg.statemem.target == nil or
                        ...
                        inst.sg:GoToState("bow_loop",
                            { target = inst.sg.statemem.target, talktask = inst.sg.statemem.talktask })
                    else
    ...
    State {
        ...
        timeline =
        {
            TimeEvent(3 * FRAMES, function(inst) inst.SoundEmitter:PlaySound(
                "dontstarve/characters/walter/woby/big/foley") end),
    ...
    State {
        ...
        timeline =
        {
            ...
            TimeEvent(4 * FRAMES, function(inst) inst.SoundEmitter:PlaySound(
                "dontstarve/characters/walter/woby/big/chuff") end),
        },
    },
    ...
    State {
        ...
        timeline =
        {
            TimeEvent(6 * FRAMES, function(inst) inst.SoundEmitter:PlaySound(
                "dontstarve/characters/walter/woby/big/bark") end),
        },
    },
    ...
    State {
        ...
        onenter = function(inst)
            inst.sg.statemem.isshield = inst.bufferedaction ~= nil and inst.bufferedaction.invobject ~= nil and
            inst.bufferedaction.invobject:HasTag("shield")
        ...
        ontimeout = function(inst)
            if inst.sg:HasStateTag("parrying") then
                ...
                inst.sg:GoToState("parry_idle",
                    { duration = inst.sg.statemem.parrytime, pauseframes = 30, talktask = talktask, isshield = inst.sg
                    .statemem.isshield })
            else
    ...
    State {
        ...
        onenter = function(inst, data)
            ...
            if data ~= nil and (data.pauseframes or 0) > 0 then
                ...
                if inst.components.playercontroller ~= nil then
                    inst.components.playercontroller:RemotePausePrediction(data.pauseframes <= 7 and data.pauseframes or
                    nil)
                end
    ...
    State {
        ...
        ontimeout = function(inst)
            if inst.sg.statemem.unequipped then
                ...
                inst.sg.statemem.parrying = true
                inst.sg:GoToState("parry_idle",
                    inst.sg.statemem.timeleft ~= nil and
                    { duration = math.max(0, inst.sg.statemem.timeleft + inst.sg.statemem.timeleft0 - GetTime()), isshield =
                    inst.sg.statemem.isshield }
    ...
    State {
        ...
        onenter = function(inst)
            ...
            if inst.components.rider:IsRiding() then
                ...
                DoTalkSound(inst)
                inst.AnimState:PlayAnimation(inst.components.inventory:IsHeavyLifting() and "heavy_refuseeat" or
                "refuseeat")
    ...
    State {
        ...
        events =
        {
            EventHandler("animover", function(inst)
                if inst.AnimState:AnimDone() then
                    inst.sg:GoToState(inst.components.inventory:GetEquippedItem(EQUIPSLOTS.HANDS) ~= nil and "item_out" or
                    "idle")
                end
            end),
        },
        ...
        timeline =
        {
            ...
            TimeEvent(17 * FRAMES, function(inst)
                if inst.sg.statemem.ispocketwatch then
                    inst.SoundEmitter:PlaySound(inst.sg.statemem.ispocketwatch_fueled and
                    "wanda2/characters/wanda/watch/weapon/pst_shadow" or "wanda2/characters/wanda/watch/weapon/pst")
                end
            end),
        },
    ...
    State {
        ...
        onenter = function(inst, mime)
            ...
            if mime then
                ...
                inst.AnimState:PushAnimation(math.random() < 0.75 and "idle_walter_storytelling" or
                "idle_walter_storytelling_2")
            end
        end,
            ...
            if inst.bufferedaction ~= nil and
                ...
                if shavee ~= nil then
                    if shavee.components.beard ~= nil then
                        pass, reason = shavee.components.beard:ShouldTryToShave(inst.bufferedaction.doer,
                            inst.bufferedaction.invobject)
                    elseif shavee.components.shaveable ~= nil then
                        pass, reason = shavee.components.shaveable:CanShave(inst.bufferedaction.doer,
                            inst.bufferedaction.invobject)
                    end
                end
            end
        ...
        timeline =
        {
            TimeEvent(0 * FRAMES, function(inst)
                if inst.components.talker ~= nil and inst.components.ghostlybond ~= nil then
                    ...
                    GetString(inst, "ANNOUNCE_ABIGAIL_SUMMON",
                        "LEVEL" .. tostring(math.max(inst.components.ghostlybond.bondlevel, 1))), nil, nil, true)
                end
            end),
            ...
            TimeEvent(51 * FRAMES, function(inst)
                inst.sg.statemem.fx = SpawnPrefab(inst.components.rider:IsRiding() and "abigailsummonfx_mount" or
                "abigailsummonfx")
    ...
    State {
        ...
        timeline =
        {
            TimeEvent(6 * FRAMES, function(inst) inst.SoundEmitter:PlaySound("dontstarve/characters/wendy/summon_pre") end),
            TimeEvent(25 * FRAMES, function(inst)
                inst.sg.statemem.fx = SpawnPrefab(inst.components.rider:IsRiding() and "abigailunsummonfx_mount" or
                "abigailunsummonfx")
                inst.sg.statemem.fx.entity:SetParent(inst.entity)
                ...
                if flower ~= nil then
                    local skin_build = flower:GetSkinBuild()
                    if skin_build ~= nil then
                        inst.sg.statemem.fx.AnimState:OverrideItemSkinSymbol("flower", skin_build, "flower", flower.GUID,
                            flower.AnimState:GetBuild())
                    end
                end
            end),
    ...
    State {
        ...
        {
            TimeEvent(30 * FRAMES, function(inst)
                local weight = inst.sg.statemem.target ~= nil and
                inst.sg.statemem.target.components.weighable:GetWeight() or nil
                if weight ~= nil and inst:PerformBufferedAction() then
                    ...
                    TUNING.WEIGHABLE_HEAVY_WEIGHT_PERCENT and "ANNOUNCE_WEIGHT_HEAVY" or "ANNOUNCE_WEIGHT"
    ...
    State {
        ...
        onenter = function(inst, repeatcast)
            ...
            local book = inst.bufferedaction ~= nil and (inst.bufferedaction.target or inst.bufferedaction.invobject) or
            nil
            if book ~= nil then
                ...
                elseif book.components.aoetargeting ~= nil then
                    --inst.sg:AddStateTag("busy")
                    inst.sg.statemem.targetfx = book.components.aoetargeting:SpawnTargetFXAt(inst.bufferedaction
    ...
    State {
        ...
        onenter = function(inst, data)
            ...
            if book ~= nil then
                ...
                if skin_build ~= nil then
                    inst.AnimState:OverrideItemSkinSymbol("book_open", skin_build, "book_open", book.GUID,
                        swap_build or "player_actions_uniqueitem", swap_prefix .. "_open")
                    inst.AnimState:OverrideItemSkinSymbol("book_closed", skin_build, "book_closed", book.GUID,
                        swap_build or "player_actions_uniqueitem", swap_prefix .. "_closed")
    ...
    State {
        ...
        onenter = function(inst)
            ...
            if book ~= nil then
                ...
                local skin_build = book:GetSkinBuild()
                if skin_build ~= nil then
                    inst.AnimState:OverrideItemSkinSymbol("book_peruse", skin_build, "book_peruse", book.GUID,
                        swap_build or "wurt_peruse", swap_prefix .. "_peruse")
                    inst.sg.statemem.symbolsoverridden = true
                elseif swap_build ~= nil then
        ...
        timeline =
        {
            TimeEvent(FRAMES, function(inst)
                ...
                if inst.sg.statemem.smashed then
                    ...
                    if prop ~= nil then
                        dist = dist + radius - .5
                        inst.sg.statemem.smashed = { prop = prop, pos = Vector3(x0 + dist * sinangle, y0,
                            z0 + dist * cosangle) }
                    else
        ...
        timeline =
        {
            ...
            TimeEvent(1 * FRAMES, function(inst)
                if inst.sg.statemem.riding then
                    ...
                    if inst.sg.statemem.ridingwoby then
                        inst.SoundEmitter:PlaySoundWithParams("dontstarve/characters/walter/woby/big/footstep",
                            { intensity = 1 })
                    end
                end
            end),
            ...
            EventHandler("animover", function(inst)
                if inst.AnimState:AnimDone() then
                    ...
                    elseif inst.sg.statemem.cometo then
                        ...
                        inst.AnimState:PlayAnimation(
                        inst.sg.statemem.isinsomniac and "insomniac_sleep_loop" or "sleep_loop", true)
                        inst.sg:AddStateTag("sleeping")
                    end
                end
            end),
                        ...
                        if mount ~= nil then
                            if mount.components.drownable ~= nil then
                                ...
                                mount:PushEvent("onsink",
                                    { noanim = true, shore_pt = Vector3(inst.components.drownable.dest_x,
    ...
    State {
        ...
        timeline =
        {
            ...
            TimeEvent(10 * FRAMES, function(inst) inst.SoundEmitter:PlaySound(
                "turnoftides/common/together/water/splash/medium") end),
        },
    ...
    State {
        ...
        onenter = function(inst)
            ...
            local target = (rod ~= nil and rod.components.oceanfishingrod ~= nil) and
            rod.components.oceanfishingrod.target or nil
            if target ~= nil and target.components.oceanfishinghook ~= nil and TUNING.OCEAN_FISHING.IDLE_QUOTE_TIME_MIN > 0 then
                inst.sg:SetTimeout(TUNING.OCEAN_FISHING.IDLE_QUOTE_TIME_MIN +
                math.random() * TUNING.OCEAN_FISHING.IDLE_QUOTE_TIME_VAR)
            end
        end,
        ...
        ontimeout = function(inst)
            if inst.components.talker ~= nil then
                ...
                inst.sg:SetTimeout(inst.sg.timeinstate + TUNING.OCEAN_FISHING.IDLE_QUOTE_TIME_MIN +
                math.random() * TUNING.OCEAN_FISHING.IDLE_QUOTE_TIME_VAR)
            end
        end,
    ...
    State {
        ...
        timeline =
        {
            ...
            TimeEvent(29 * FRAMES, function(inst)
                if inst.components.talker ~= nil then
                    inst.components.talker:Say(
                    GetString(inst, inst.sg.statemem.escaped_str or "ANNOUNCE_OCEANFISHING_LINESNAP"), nil, nil, true)
                end
            end),
        },
        ...
        onenter = function(inst, data)
            ...
            if attacker ~= nil and attacker:IsValid() then
                ...
                if mount ~= nil then
                    ...
                    local radius = attacker.Physics ~= nil and attacker.Physics:GetRadius() or
                    attacker:GetPhysicsRadius(0)
                    if radius > 0 then
    ...
    State {
        ...
        onenter = function(inst)
            inst.components.locomotor:Stop()
            inst.AnimState:PlayAnimation(inst.components.inventory:IsHeavyLifting() and "heavy_jump_pre" or "jump_pre",
                false)
        end,
    ...
    State {
        ...
        onenter = function(inst)
            ...
            if staff ~= nil and staff.components.aoetargeting ~= nil then
                local buffaction = inst:GetBufferedAction()
                if buffaction ~= nil then
                    inst.sg.statemem.targetfx = staff.components.aoetargeting:SpawnTargetFXAt(buffaction
                    :GetDynamicActionPoint())
                    if inst.sg.statemem.targetfx ~= nil then
                        inst.sg.statemem.targetfx:ListenForEvent("onremove", OnRemoveCleanupTargetFX, inst)
                    end
    ...
    State {
        ...
        {
            TimeEvent(7 * FRAMES, function(inst)
                inst.sg.statemem.stafffx = SpawnPrefab((inst.components.rider ~= nil and inst.components.rider:IsRiding()) and
                "cointosscastfx_mount" or "cointosscastfx")
                inst.sg.statemem.stafffx.entity:SetParent(inst.entity)
        ...
        onenter = function(inst, repeatcast)
            ...
            if item then
                ...
                if item.components.aoetargeting and not (item.components.spellbook and item.components.spellbook:HasSpellFn()) then
                    ...
                    inst.sg.statemem.targetfx = item.components.aoetargeting:SpawnTargetFXAt(inst.bufferedaction
                    :GetDynamicActionPoint())
    ...
    State {
        ...
        onenter = function(inst, targetfx)
            ...
            if item then
                ...
                if skin_build then
                    inst.AnimState:OverrideItemSkinSymbol("swap_remote", skin_build, "swap_remote", item.GUID, swap_build)
                    inst.AnimState:OverrideItemSkinSymbol("remote_overlay", skin_build, "remote_overlay", item.GUID,
                        swap_build)
                else
                    inst.AnimState:OverrideSymbol("swap_remote", swap_build, "swap_remote")
        ...
        onenter = function(inst, data)
            if data ~= nil and
                ...
                if data.weapon.components.aoeweapon_lunge:DoLunge(inst, pos, data.targetpos) then
                    inst.SoundEmitter:PlaySound(data.weapon.components.aoeweapon_lunge.sound or
                    "dontstarve/common/lava_arena/fireball")
    ...
    State {
        ...
        onenter = function(inst)
            ...
            if weapon ~= nil and weapon.components.aoetargeting ~= nil then
                local buffaction = inst:GetBufferedAction()
                if buffaction ~= nil then
                    inst.sg.statemem.targetfx = weapon.components.aoetargeting:SpawnTargetFXAt(buffaction
                    :GetDynamicActionPoint())
                    if inst.sg.statemem.targetfx ~= nil then
                        inst.sg.statemem.targetfx:ListenForEvent("onremove", OnRemoveCleanupTargetFX, inst)
                    end
            ...
            if data ~= nil then
                ...
                if data ~= nil and
                    ...
                    if inst.sg.statemem.startingpos.x ~= data.targetpos.x or inst.sg.statemem.startingpos.z ~= data.targetpos.z then
                        ...
                        inst.Physics:SetMotorVel(
                        math.sqrt(distsq(inst.sg.statemem.startingpos.x, inst.sg.statemem.startingpos.z, data.targetpos
                        .x, data.targetpos.z)) / (12 * FRAMES), 0, 0)
                    end
                    return
                end
            end
        ...
        timeline =
        {
            ...
            TimeEvent(13 * FRAMES, function(inst)
                ...
                if inst.sg.statemem.weapon:IsValid() then
                    inst.sg.statemem.weapon.components.aoeweapon_leap:DoLeap(inst, inst.sg.statemem.startingpos,
    ...
    State {
        ...
        events =
        {
            EventHandler("animover", function(inst)
                if inst.AnimState:AnimDone() then
                    inst:Hide()
                    inst.Physics:Teleport(inst.sg.statemem.data.data.targetpos.x, 0,
                        inst.sg.statemem.data.data.targetpos.z)
                end
            end),
        },
        ...
        timeline =
        {
            ...
            FrameEvent(4, function(inst)
                ...
                if equip then
                    ...
                    local ammo = equip.components.container and
                    equip.components.container:GetItemInSlot(equip.components.container:GetNumSlots()) or nil
                    if ammo == nil then
                        inst.sg.mem.slingshotchain = nil
                        inst:ClearBufferedAction()
                        inst.components.talker:Say(GetString(inst, "ANNOUNCE_SLINGHSOT_OUT_OF_AMMO"))
                    ...
                    if inst.sg.statemem.chargeticks == nil then
                        inst.sg.mem.slingshotchain = inst.sg.statemem.chain
                        if inst.sg.statemem.chainspeedfx then
                            SpawnPrefab(inst.components.rider and inst.components.rider:IsRiding() and
                            "slingshot_powerup_mounted_fx" or "slingshot_powerup_fx").entity:SetParent(inst.entity)
                        end
                        inst:PerformBufferedAction()
                        inst.SoundEmitter:PlaySound("dontstarve/characters/walter/slingshot/shoot")
                        return
    ...
    State {
        ...
        onupdate = function(inst)
            if inst.sg.statemem.weapon == nil then
                inst.sg.statemem.isreleased = inst.sg.statemem.isreleased or
                not (inst.components.playercontroller and inst.components.playercontroller:IsAnyOfControlsPressed(CONTROL_SECONDARY, CONTROL_CONTROLLER_ALTACTION))
        ...
        onenter = function(inst)
            ...
            if pos ~= nil then
                ...
                if equip ~= nil and equip.components.aoetargeting ~= nil then
                    inst.sg.statemem.targetfx = equip.components.aoetargeting:SpawnTargetFXAt(buffaction
                    :GetDynamicActionPoint())
                    if inst.sg.statemem.targetfx ~= nil then
                        inst.sg.statemem.targetfx:ListenForEvent("onremove", OnRemoveCleanupTargetFX, inst)
                    end
            ...
            if (inst.sg.statemem.projectiledelay or 0) > 0 then
                ...
                if inst.sg.statemem.projectiledelay <= 0 then
                    ...
                    if inst:PerformBufferedAction() and inst.sg.statemem.targetfx ~= nil then
                        if inst.sg.statemem.targetfx:IsValid() then
                            inst.sg.statemem.targetfx:RemoveEventCallback("onremove", OnRemoveCleanupTargetFX, inst)
                            inst.sg.statemem.targetfx:DoTaskInTime(1.05,
                                inst.sg.statemem.targetfx.KillFX or inst.sg.statemem.targetfx.Remove)
                        end
                        inst.sg.statemem.targetfx = nil
                    end
                end
        ...
        onenter = function(inst, data)
            ...
            elseif data.sound ~= false then
                if (data.sounddelay or 0) <= 0 then
                    ...
                    inst.sg.statemem.emotesoundtask = inst:DoTaskInTime(data.sounddelay, DoEmoteSound, data
                    .soundoverride, data.soundlooped)
                end
            end
                ...
                if mount ~= nil and mount.sounds ~= nil and mount.sounds[data.mountsound] ~= nil then
                    if (data.mountsoundperiod or 0) <= 0 then
                        if (data.mountsounddelay or 0) <= 0 then
                            ...
                            inst.sg.statemem.emotemountsoundtask = inst:DoTaskInTime(data.mountsounddelay,
                                DoForcedEmoteSound, mount.sounds[data.mountsound])
                        end
                    elseif (data.mountsounddelay or 0) <= 0 then
                        inst.sg.statemem.emotemountsoundtask = inst:DoPeriodicTask(data.mountsoundperiod,
                            DoForcedEmoteSound, nil, mount.sounds[data.mountsound])
                        inst.SoundEmitter:PlaySound(mount.sounds[data.mountsound])
                    else
                        inst.sg.statemem.emotemountsoundtask = inst:DoPeriodicTask(data.mountsoundperiod,
                            DoForcedEmoteSound, data.mountsounddelay, mount.sounds[data.mountsound])
                    end
                end
            end
                ...
                if mount ~= nil and mount.sounds ~= nil and mount.sounds[data.mountsound2] ~= nil then
                    if (data.mountsound2period or 0) <= 0 then
                        if (data.mountsound2delay or 0) <= 0 then
                            ...
                            inst.sg.statemem.emotemountsound2task = inst:DoTaskInTime(data.mountsound2delay,
                                DoForcedEmoteSound, mount.sounds[data.mountsound2])
                        end
                    elseif (data.mountsound2delay or 0) <= 0 then
                        inst.sg.statemem.emotemountsound2task = inst:DoPeriodicTask(data.mountsound2period,
                            DoForcedEmoteSound, nil, mount.sounds[data.mountsound2])
                        inst.SoundEmitter:PlaySound(mount.sounds[data.mountsound2])
                    else
                        inst.sg.statemem.emotemountsound2task = inst:DoPeriodicTask(data.mountsound2period,
                            DoForcedEmoteSound, data.mountsound2delay, mount.sounds[data.mountsound2])
                    end
                end
            end
    ...
    State {
        ...
        onenter = function(inst)
            inst.sg.statemem.heavy = inst.components.inventory:IsHeavyLifting()
            inst.sg.statemem.ridingwoby = inst.components.rider.target_mount and
            inst.components.rider.target_mount:HasTag("woby")
    ...
    State {
        ...
        onenter = function(inst)
            inst.components.locomotor:Stop()
            inst:PerformBufferedAction()
            inst.AnimState:PlayAnimation(inst:IsChannelCastingItem() and "channelcast_idle_pre" or
            "channelcast_oh_idle_pre")
            inst.sg:GoToState("idle", true)
        end,
    },
    ...
    State {
        ...
        onenter = function(inst)
            inst.components.locomotor:Stop()
            inst.AnimState:PlayAnimation(inst:IsChannelCastingItem() and "channelcast_idle_pst" or
            "channelcast_oh_idle_pst")
            inst:PerformBufferedAction()
            inst.sg:GoToState("idle", true)
        end,
        ...
        timeline =
        {
            TimeEvent(8 * FRAMES, function(inst)
                ...
                if pocketwatch ~= nil and pocketwatch:IsValid() and pocketwatch.components.pocketwatch:CanCast(inst, inst.sg.statemem.target) then
                    inst.sg.statemem.stafffx = SpawnPrefab((inst.components.rider ~= nil and inst.components.rider:IsRiding()) and
                    "pocketwatch_cast_fx_mount" or "pocketwatch_cast_fx")
                    inst.sg.statemem.stafffx.entity:SetParent(inst.entity)
    ...
    State {
        ...
        onenter = function(inst, data)
            ...
            inst.sg.statemem.warpback_data = data
            .warpback                             -- 'warpback' passed in through the previous state bug is set by the action function
                    ...
                    if dest_worldid ~= nil and dest_worldid ~= TheShard:GetShardId() then
                        if Shard_IsWorldAvailable(dest_worldid) then
                            TheWorld:PushEvent("ms_playerdespawnandmigrate",
                                { player = inst, portalid = nil, worldid = dest_worldid, x = warpback_data.dest_x, y =
                                warpback_data.dest_y, z = warpback_data.dest_z })
                        else
                            warpback_data.dest_x, warpback_data.dest_y, warpback_data.dest_z = inst.Transform
                            :GetWorldPosition()
    ...
    State {
        ...
        onenter = function(inst)
            ...
            if watch ~= nil then
                ...
                inst.sg.statemem.same_shard = watch.components.recallmark ~= nil and
                watch.components.recallmark:IsMarkedForSameShard()
            end
        end,
        ...
        timeline =
        {
            ...
            TimeEvent(32 * FRAMES, function(inst)
                if inst.sg.statemem.action_failed then
                    ...
                    local line = inst.sg.statemem.same_shard and "ANNOUNCE_POCKETWATCH_OPEN_PORTAL" or
                    "ANNOUNCE_POCKETWATCH_OPEN_PORTAL_DIFFERENTSHARD"
        ...
        timeline =
        {
            TimeEvent(3 * FRAMES, function(inst)
                ...
                if songdata then
                    ...
                    ("dontstarve_DLC001/characters/wathgrithr/" .. (songdata.INSTANT and "quote" or "sing")))
                end
            end),
    ...
    State {
        ...
        onenter = function(inst)
            local buffaction = inst:GetBufferedAction()
            local required_skill = buffaction and buffaction.invobject.songdata and
            buffaction.invobject.songdata.REQUIRE_SKILL or nil
    ...
    State {
        ...
        onupdate = function(inst, dt)
            ...
            if inst.sg.statemem.feather_fx <= 0 then
                inst.sg.statemem.feather_fx = 7 * FRAMES
                SpawnPrefab("weregoose_feathers" .. tostring(math.random(3))).Transform:SetPosition(inst.sg.statemem.pos
                :Get())
            end
    ...
    State {
        ...
        onexit = function(inst)
            ...
            if not inst.sg.statemem.keep_eating then
                TheWorld:PushEvent("feasterfinished",
                    { player = inst, target = target, is_in_dark = inst.sg.statemem.is_in_dark })
            end
        end,
    },
    ...
    State {
        ...
        timeline =
        {
            TimeEvent(26 * FRAMES, function(inst) inst.SoundEmitter:PlaySound(
                "dontstarve/characters/woodie/goose/death_voice") end),
    ...
    State {
        ...
        onenter = function(inst, data)
            ...
            if inst.components.magician ~= nil then
                ...
                equipped = inst.components.magician.equip and
                inst.components.inventory:GetEquippedItem(EQUIPSLOTS.HEAD) == nil
                if locomoting then
                    inst.components.magician:StopUsing()
                end
            end
    ...
    State {
        ...
        events =
        {
            EventHandler("ontalk", function(inst)
                ...
                local duration = inst.sg.statemem.talktask ~= nil and GetTaskRemaining(inst.sg.statemem.talktask) or
                1.5 + math.random() * .5
                if inst:HasTag("mime") then
                ...
                then
                    ...
                    if data.sound then --sound might be a boolean, so don't do ~= nil
                        if (data.sounddelay or 0) <= 0 then
                            ...
                            inst.sg.statemem.emotesoundtask = inst:DoTaskInTime(data.sounddelay, DoForcedEmoteSound,
                                data.sound)
                        end
                    elseif data.sound ~= false then
                        if (data.sounddelay or 0) <= 0 then
                            ...
                            inst.sg.statemem.emotesoundtask = inst:DoTaskInTime(data.sounddelay, DoEmoteSound,
                                data.soundoverride, data.soundlooped)
                        end
                    end
                end
                return true
...
local hop_timelines =
{
    ...
    {
        TimeEvent(0, function(inst)
            inst.components.embarker.embark_speed = math.clamp(
            inst.components.locomotor:RunSpeed() * inst.components.locomotor:GetSpeedMultiplier() +
            TUNING.WILSON_EMBARK_SPEED_BOOST, TUNING.WILSON_EMBARK_SPEED_MIN, TUNING.WILSON_EMBARK_SPEED_MAX)
        end),
    },
    hop_loop =
    {
...
local hop_anims =
{
    pre = function(inst) return (inst.replica.inventory ~= nil and inst.replica.inventory:IsHeavyLifting() and (inst.replica.rider == nil or not inst.replica.rider:IsRiding())) and
        "boat_jumpheavy_pre" or "boat_jump_pre" end,
    loop = function(inst) return (inst.replica.inventory ~= nil and inst.replica.inventory:IsHeavyLifting() and (inst.replica.rider == nil or not inst.replica.rider:IsRiding())) and
        "boat_jumpheavy_loop" or "boat_jump_loop" end,
    pst = function(inst) return (inst.replica.inventory ~= nil and inst.replica.inventory:IsHeavyLifting() and (inst.replica.rider == nil or not inst.replica.rider:IsRiding())) and
        "boat_jumpheavy_pst" or "boat_jump_pst" end,
}

CommonStates.AddRowStates(states, false)
CommonStates.AddHopStates(states, true, hop_anims, hop_timelines, "turnoftides/common/together/boat/jump_on",
    landed_in_falling_state, { start_embarking_pre_frame = 4 * FRAMES })

local GymStates = require("stategraphs/SGwilson_gymstates")
GymStates.AddGymStates(states, actionhandlers, events)

if TheNet:GetServerGameMode() == "quagmire" then
    event_server_data("quagmire", "stategraphs/SGwilson").AddQuagmireStates(states, DoTalkSound, StopTalkSound,
        ToggleOnPhysics, ToggleOffPhysics)
end
